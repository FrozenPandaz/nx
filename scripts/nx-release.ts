 ...
 */
execSync('rm -rf build/npm && mkdir -p build/npm', {
  stdio: [0, 1, 2],
});
execSync('cp -R build/packages/* build/npm', {
  stdio: [0, 1, 2],
});
/**
 * Get rid of tarballs at top of copied directory (made with npm pack)
 */
execSync(`find build/npm -maxdepth 1 -name "*.tgz" -delete`, {
  stdio: [0, 1, 2],
});
...
process.env.GH_TOKEN = !parsedArgs.local
  ? process.env.GH_TOKEN ?? process.env.GITHUB_TOKEN_RELEASE_IT_NX
  : 'dummy-gh-token';
/**
