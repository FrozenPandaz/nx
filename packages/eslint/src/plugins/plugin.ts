...
function getProjectsUsingESLintConfig(
  ...
  // You can do this outside and pass it in
  // Create a Set of all the directories containing eslint configs
  const eslintRoots = new Set(context.configFiles.map(dirname));

  // Remove this one from the set so the set only contains directories of other eslint configs
  // eslintRoots.delete(dirname(configFilePath));
