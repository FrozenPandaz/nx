...
import { parseTarget, editTarget } from '../../src/utils/cli-config-utils';
import {
...
function updateKarmaConfigs(host: Tree, context: SchematicContext) {
  ...
  if (functionCall) {
    ...
    const coverageAssignment = findPropertyAssignment(
      nodes,
      'coverageIstanbulReporter'
    );

    if (coverageAssignment) {
      if (ts.isObjectLiteralExpression(coverageAssignment.initializer)) {
        const objectLiteralElement = coverageAssignment.initializer;
        objectLiteralElement.properties
          .filter(
            property =>
              ts.isPropertyAssignment(property) &&
              ts.isIdentifier(property.name) &&
              property.name.text === 'dir'
          )
          .forEach((property: ts.PropertyAssignment) => {
            newContents = newContents.replace(
              property.initializer.getText(sourceFile),
              `require('path').join(__dirname, '../../coverage')`
            );
          });
      }
    }

    const angularCliAssignment = findPropertyAssignment(nodes, 'angularCli');

    if (angularCliAssignment) {
      newContents = newContents.replace(
        angularCliAssignment.getText(sourceFile),
        ''
      );
    }
  }
...
function updateProtractorConfigs(host: Tree, context: SchematicContext) {
  ...
  const specDeclaration = findPropertyAssignment(nodes, 'specs');
  if (specDeclaration) {
    newContents = newContents.replace(
  ...
  Object.entries<any>(angularJson.projects)
    .filter(
      ([key, project]) =>
        ...
        project.architect.build.options.main.startsWith('libs')
    )
    .forEach(([key]) => {
      console.log(key);
      fs.renameSync(`libs/${key}`, `libs/${key}/lib`);
    });
...
const updateAngularJson = updateJsonInTree('angular.json', json => {
  ...
  json.defaultProject = json.defaultProject.replace('/', '-');
  ...
  Object.entries<any>(json.projects).forEach(([key, project]) => {
    ...
    switch (type) {
      case 'application':
        ...
        project.architect.serve.options.browserTarget = editTarget(
          project.architect.serve.options.browserTarget,
          target => {
            return {
              ...target,
              project: newKey
        ...
        project.architect.serve.configurations.production.browserTarget = editTarget(
        ...
        project.architect['extract-i18n'].options.browserTarget = editTarget(
      ...
      case 'library':
        ...
        break;
      case 'e2e':
        project.architect.e2e.options.devServerTarget = editTarget(
          project.architect.e2e.options.devServerTarget,
          target => {
            return {
              ...target,
              project: target.project.replace('/', '-')
            };
          }
        );
    ...
    // Rename projects with slashes if the project still exists.
    if (key.includes('/') && !!json.projects[key]) {
      json.projects[newKey] = project;
      delete json.projects[key];
    }
  });
  return json;
